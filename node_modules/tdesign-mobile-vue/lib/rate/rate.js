...
import { defineComponent, ref, toRefs, computed, createVNode, h } from 'vue';
...
var _Rate = defineComponent({
  ...
  setup: function setup(props2, context) {
    ...
    var _useVModel = useVModel(value, modelValue, props2.defaultValue, props2.onChange),
      ...
      actualVal = _useVModel2[0],
      setActualVal = _useVModel2[1];
    ...
    var unitConvert = function unitConvert(value2) {
    ...
    var getRect = function getRect(event, eventType) {
      if (rateWrapper.value) {
        ...
        var value2 = remainder <= 0.5 && allowHalf ? integral + 0.5 : integral + 1;
        if (value2 > count) {
          value2 = count;
        } else if (value2 < 0) {
          value2 = 0;
        }
        popoverValue.value = value2;
        var fontSize = size || getComputedStyle(rateWrapper.value).getPropertyValue("font-size");
        var leftDis = Math.ceil(value2 - 1) * (unitConvert(gap) + unitConvert(fontSize)) + unitConvert(fontSize) * 0.5;
        tipsVisible.value = true;
        tipsLeft.value = Math.max(leftDis, unitConvert(fontSize) * 0.5);
        scaleIndex.value = Math.ceil(value2);
    ...
    var onSelect = function onSelect(value2) {
      setActualVal(value2);
      hideTips();
    };
    return function () {
      var iconComponent = function iconComponent(n, value2) {
        var classPrefix = "".concat(rateClass.value, "__icon-left");
        var select = value2 >= n;
        var selectHalf = Math.ceil(value2) >= n;
