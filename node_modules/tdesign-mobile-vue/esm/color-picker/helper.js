...
import { DEFAULT_SYSTEM_SWATCH_COLORS } from '../_common/js/color-picker/constants.js';
...
var getFormatList = function getFormatList(format, color) {
  var FORMAT_MAP = {
    ...
    CMYK: [].concat(_toConsumableArray(Object.values(color.getCmyk())), [0]),
    HEX8: [color.hex8, 0],
    CSS: [color.css, 0],
    HEX: [color.hex, 0]
  };
  ...
  if (cur) {
    return [format, [].concat(_toConsumableArray(cur.slice(0, cur.length - 1)), ["".concat(Math.round(color.alpha * 100), "%")])];
  }
...
export { genSwatchList, getCoordinate, getFormatList };
