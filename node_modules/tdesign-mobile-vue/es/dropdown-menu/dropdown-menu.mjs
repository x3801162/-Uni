...
import { get, camelCase, isFunction } from 'lodash-es';
import config from '../config.mjs';
import { context, DropdownMenuExpandState } from './context.mjs';
import useExpose from '../hooks/useExpose.mjs';
...
var _DropdownMenu = defineComponent({
  ...
  setup: function setup(props, _ref) {
    ...
    var menuTitles = computed(function () {
      var _menuItems$value;
      return (_menuItems$value = menuItems.value) === null || _menuItems$value === void 0 ? void 0 : _menuItems$value.map(function (item, index) {
        var _keys$label2;
        ...
        var target = options === null || options === void 0 ? void 0 : options.find(function (item2) {
          var _keys$value;
          return get(item2, (_keys$value = keys === null || keys === void 0 ? void 0 : keys.value) !== null && _keys$value !== void 0 ? _keys$value : "value") === currentValue;
        });
        if (state.itemsLabel.length < index + 1) {
          var _keys$label;
          var targetLabel = target && get(target, (_keys$label = keys === null || keys === void 0 ? void 0 : keys.label) !== null && _keys$label !== void 0 ? _keys$label : "label") || "";
        ...
        return {
          labelProps: label,
          label: label || get(target, (_keys$label2 = keys === null || keys === void 0 ? void 0 : keys.label) !== null && _keys$label2 !== void 0 ? _keys$label2 : "label"),
          disabled: disabled !== void 0 && disabled !== false,
          icon: icon
        };
      });
    ...
    var collapseMenu = function collapseMenu() {
      menuTitles.value.forEach(function (_item, index) {
    ...
    var control = {
      ...
      emitEvents: function emitEvents(emit, trigger) {
        var eventHandler = props["on".concat(camelCase(emit))];
        eventHandler === null || eventHandler === void 0 || eventHandler(trigger);
      }
    };
    ...
    return function () {
      ...
      var renderDropdownItemIcon = function renderDropdownItemIcon(item, idx) {
        var icon = item.icon;
        var isActive = idx === state.activeId;
        var iconClass = [styleIcon.value(item, idx)];
        if (icon) {
          var isArray = Array.isArray(icon);
          var isTwoIcons = isArray && icon.length === 2;
          var selectedIcon;
          if (isTwoIcons) {
          ...
          var iconContent = isFunction(selectedIcon) ? selectedIcon() : selectedIcon;
          return createVNode("div", {
            "class": iconClass
          }, [iconContent]);
        }
        return props.direction === "down" ? createVNode(resolveComponent("caret-down-small-icon"), {
          "class": iconClass
        }, null) : createVNode(resolveComponent("caret-up-small-icon"), {
          "class": iconClass
        }, null);
      };
      return createVNode("div", {
        ...
        }, [item.labelProps || item.label]), renderDropdownItemIcon(item, idx)]);
      }), defaultSlot]);
    };
  }
});
