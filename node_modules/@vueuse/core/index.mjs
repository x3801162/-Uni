...
import { isRef, ref, shallowRef, watchEffect, computed, inject, isVue3, version, defineComponent, h, shallowReactive, Fragment, watch, getCurrentInstance, customRef, onUpdated, onMounted, readonly, nextTick, reactive, markRaw, unref, getCurrentScope, isVue2, set, del, isReadonly, onBeforeUpdate } from 'vue-demi';
// TransitionGroup is not available in UniApp, so we'll handle it conditionally
let TransitionGroup;
try {
  // Try to import TransitionGroup separately
  const vueDemi = require('vue-demi');
  TransitionGroup = vueDemi.TransitionGroup;
} catch (e) {
  // If import fails, set to null
...
function createTemplatePromise(options = {}) {
  ...
  const component = /* #__PURE__ */ defineComponent((_, { slots }) => {
    ...
    if (options.transition && TransitionGroup)
